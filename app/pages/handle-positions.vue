<template>
  <div>
    <CmsPageHeader title="Handle positions" description="Define handle position presets with optional reference images.">
      <template #actions>
    ...
    <BasePanel :pending="pending" :error="!!error" :pagination="pagination" :empty-first-page="rows.length === 0 && page === 1" :empty-off-page="rows.length === 0 && page > 1" :item-count="rows.length" :page="page">
      <template #toolbar>
        <span class="base-panel__summary">
          ...
          <template v-else>{{ pagination!.total }} {{ pagination!.total === 1 ? 'position' : 'positions' }}</template>
        </span>
      ...
      <template #loading>Loading handle positions&hellip;</template>
      <template #error>
        <Icon name="lucide:alert-triangle" class="base-panel__alert-icon" />
        <span>Failed to load handle positions.</span>
      ...
      <template #empty>
        ...
        <h3 class="base-panel__empty-title">No handle positions yet</h3>
        <p class="base-panel__empty-desc">Create Strapi &ldquo;HandlePosition&rdquo; entries with a name and optional image.</p>
        <BaseButton type="button" @click="openCreateModal">
          <Icon name="lucide:plus" class="base-btn__icon" />
      ...
      <template #empty-offpage>
        <p class="base-panel__empty-page">No positions on this page.</p>
      ...
      <template #pagination>
        <BasePagination v-model:page="page" :page-count="pagination!.pageCount" :disabled="pending" aria-label="Handle position pages" variant="panel" />
      </template>
    </BasePanel>

    <ModalHandlePosition ref="modalRef" @saved="onSaved" />
  </div>
</template>
...
import {
  defaultHandlePositionsResponse,
  deleteHandlePosition,
  handlePositionsListPath,
  handlePositionsListQuery,
  type HandlePosition,
  type HandlePositionsResponse,
...
const { data, pending, error, refresh } = useFetch<HandlePositionsResponse>(handlePositionsListPath, {
  key: computed(() => `handle-positions-p${page.value}`),
  query: computed(() => handlePositionsListQuery(page.value, PAGE_SIZE)),
  default: () => defaultHandlePositionsResponse(PAGE_SIZE),
});
...
async function confirmBulkDelete() {
  ...
  const noun = ids.length === 1 ? 'handle position' : 'handle positions';
  const ok = await requestConfirm({
