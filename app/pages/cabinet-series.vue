<template>
  <div>
    <CmsPageHeader title="Cabinet Series" description="Manage cabinet series with codes, dimensions, product lines, and linked subcategories.">
      <template #actions>
    ...
    <BasePanel :pending="pending" :error="!!error" :pagination="pagination" :empty-first-page="rows.length === 0 && page === 1" :empty-off-page="rows.length === 0 && page > 1" :item-count="rows.length" :page="page">
      ...
      <template #loading>Loading cabinet series&hellip;</template>
      <template #error>
      ...
      <template #empty>
        ...
        <h3 class="base-panel__empty-title">No cabinet series yet</h3>
        <p class="base-panel__empty-desc">Create your first cabinet series to start organizing cabinet types and variants.</p>
        <BaseButton type="button" @click="openCreateModal">
          <Icon name="lucide:plus" class="base-btn__icon" />
          Create series
        </BaseButton>
      ...
      <template #pagination>
        <BasePagination v-model:page="page" :page-count="pagination!.pageCount" :disabled="pending" aria-label="Cabinet series pages" variant="panel" />
      </template>
    </BasePanel>
...
const { data, pending, error, refresh } = useFetch<CabinetSeriesListResponse>(cabinetSeriesListPath, {
  key: computed(() => `cabinet-series-p${page.value}`),
...
function subcategoryLabel(row: CabinetSeries): string {
  ...
  if (!sc) return '—';
  if ('name' in sc) return (sc as { name: string }).name;
  if ('data' in sc && sc.data) return sc.data.name;
  return '—';
}
