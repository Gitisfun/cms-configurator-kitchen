import { buildCabinetSeriesData } from '../../utils/cabinetSeriesPayload';
...
export default defineEventHandler(async (event) => {
  ...
  try {
    data = buildCabinetSeriesData(body);
  } catch (e: unknown) {
  ...
  try {
    return await $fetch(`${config.strapiUrl}/api/cabinet-series/${encodeURIComponent(documentId)}`, {
      method: 'PUT',
      headers: {
