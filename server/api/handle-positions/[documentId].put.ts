import { buildHandlePositionData } from '../../utils/handlePositionPayload';
...
export default defineEventHandler(async (event) => {
  ...
  try {
    data = buildHandlePositionData(body);
  } catch (e: unknown) {
  ...
  try {
    return await $fetch(
      `${config.strapiUrl}/api/handle-positions/${encodeURIComponent(documentId)}`,
      {
        method: 'PUT',
        headers: {
