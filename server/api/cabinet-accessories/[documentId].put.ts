import { buildCabinetAccessoryData } from '../../utils/cabinetAccessoryPayload';
...
export default defineEventHandler(async (event) => {
  ...
  try {
    data = buildCabinetAccessoryData(body);
  } catch (e: unknown) {
  ...
  try {
    return await $fetch(`${config.strapiUrl}/api/cabinet-accessories/${encodeURIComponent(documentId)}`, {
      method: 'PUT',
      headers: {
